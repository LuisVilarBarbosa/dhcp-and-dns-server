...
AUXILIARY_FILES_PATH=$(echo "$(pwd)/$0" | grep -P ".*/" -o)
...
sudo su -c "cat \"$AUXILIARY_FILES_PATH/etc-network-interfaces\" >> /etc/network/interfaces"
...
sudo cp -f "$AUXILIARY_FILES_PATH/dhcp/dhcpd.conf" /etc/dhcp/
sudo cp -f "$AUXILIARY_FILES_PATH/dhcp/isc-dhcp-server" /etc/network/if-up.d/

sudo /usr/sbin/rndc-confgen -a
sudo cp -f "$AUXILIARY_FILES_PATH/dns/named.conf.options" /etc/bind/
sudo cp -f "$AUXILIARY_FILES_PATH/dns/named.conf.local" /etc/bind/
sudo cp -f "$AUXILIARY_FILES_PATH/dns/db.home" /etc/bind/
sudo chown bind:bind /etc/bind/db.home
sudo cp -f "$AUXILIARY_FILES_PATH/dns/db.1.168.192.rev" /etc/bind/
